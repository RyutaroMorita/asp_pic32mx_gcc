 ...
 *  非タスクコンテキスト用のスタック初期値
 */
#define TOPPERS_ISTKPT(istk, istksz) (((STK_T *)((uint8_t *)(istk) + (istksz))) - 1)

/*
 *  タスクコンテキストブロックの定義
 */
typedef struct task_context_block {
	intptr_t	*msp;		/* スタックポインタ */
	FP			pc;			/* プログラムカウンタ */
} TSKCTXB;
 ...
 */
#define EXT_IPM(iipm)	(-CAST(PRI, (iipm) >> 10))		/* 外部表現に変換 */
#define INT_IPM(ipm)	(CAST(uint32_t, -(ipm)) << 10)	/* 内部表現に変換 */
...
//	return(current_sr() & 0x0700U);
	return(current_sr() & 0x00001C00UL);
}
...
//	set_sr((current_sr() & ~0x0700U) | iipm);
	set_sr((current_sr() & ~0x00001C00UL) | iipm);
}
...
extern volatile uint32_t	saved_iipm;	/* 割込み優先度マスクを保存する変数 */
...
{
	return(*((uint32_t *) p_excinf) & 0x00001C00UL);
}
